...
class Node:
    def __init__(self, id, dim=8):
        ...
        self.in_deg = 0
        self.age = 1  # node lifetime (Sybil defense)
...
def query_nodes(node, nodes):
    ...
    for n in candidates:
        ...
        # ★ Sybil resistance: penalize low-age nodes
        age_factor = np.log(n.age + 1)

        score = (0.5 * n.trust + 0.5 * novelty) * age_factor
...
def search_nodes(node, nodes):
    ...
    for n in candidates:
        sim = essence_similarity(node, n)
        age_factor = np.log(n.age + 1)
        score = (0.6 * n.trust + 0.4 * sim) * age_factor
        scored.append((score, n))
