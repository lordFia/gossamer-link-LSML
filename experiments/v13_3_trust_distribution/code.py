# v13.3.4 trust distribution flattening (anti-saturation + target spacing)
...
def time_factor(node, t):
    return np.exp(-(t - node.last_active) * TRUST_DECAY)
...
def update_trust(nodes, neighbors_dict):
    ...
    # ★ v13.3.4: target spacing
    target = ranks / (len(nodes) - 1 + 1e-8)  # 0〜1
    target = 0.05 + 0.9 * target              # map to [0.05, 0.95]

    for i, node in enumerate(nodes):
        node.trust = float(np.clip(
            (1 - TRUST_DECAY) * node.trust +
            0.4 * (target[i] - node.trust),   # pull toward evenly spaced target
            0.05, 0.95
        ))
